...
REQUIRED_MODULES = ["numpy", "networkx", "scipy", "h5py"]
...
def check_dependencies(root: Path) -> None:
    ...
    msg = {
        ...
        "fix": [
            ...
            "or run: ./setup_env.sh && venv/bin/python tools/run_whole_anti_reafference_suite.py --reset --run",
        ],
    }
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(
        description="Run the anti-reafference experiment as three whole 1500-tick runs, no burst workflow."
    )
    ap.add_argument("--run", action="store_true", help="run all three full runs and analyze")
    ...
    ap.add_argument("--suite-dir", type=Path, default=Path("runs/anti_reafference_whole_1500"))
    ...
    import run_clean_anti_reafference_suite as clean
    ...
    if ns.run:
        print(json.dumps({
            "mode": "whole_run_three_experiment_suite",
            "suite_dir": str(args.suite_dir),
            "ticks_total_per_run": cfg["ticks_total"],
            "switch_tick": cfg["switch_tick"],
            "runs": clean.RUN_NAMES,
            "note": "Each run is executed as one continuous run. No 300-tick burst/reload workflow is used.",
        }, indent=2), flush=True)
        for run_name in clean.RUN_NAMES:
            print(f"\n=== full run: {run_name} ===\n", flush=True)
            clean.run_one_burst(args, cfg, run_name)
        print("\n=== analysis ===\n", flush=True)
        print(json.dumps(clean.analyze_suite(args, cfg), indent=2))
        return 0
