...
def main() -> int:
    ...
    ap.add_argument("--run", action="store_true", help="run all six full runs and analyze")
    ...
    ap.add_argument("--tick-print-stride", type=int, default=1, help="print every N ticks; witness ticks always print")
    ...
    if ns.run:
        print(json.dumps({
            ...
            "note": "Each run is executed as one continuous run. No warmup exclusion is applied by the runner.",
        }, indent=2), flush=True)
