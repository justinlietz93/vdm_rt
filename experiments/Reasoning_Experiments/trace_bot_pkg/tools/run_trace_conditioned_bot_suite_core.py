...
def live_witness_block(cfg: Dict[str, Any], *, tick: int, run_name: str, ev: Dict[str, Any], policy: str,
    ...
    if not live_enabled(cfg):
        return
    topn = max(1, int(cfg.get("live_topn", 3)))
...
def plan_for(run_name: str, tick: int, switch_tick: int) -> Tuple[str, str, str]:
    ...
    if run_name == "bot_matched_trace":
        return "bot_matched_trace_full", "bot_matched_trace", "bot_matched_trace"
    if run_name == "bot_lagged_trace":
        return "bot_lagged_trace_full", "bot_lagged_trace", "bot_lagged_trace"
    if run_name == "anti_trace_return":
        return "anti_trace_return_full", "anti_vector", "anti_vector"
    if run_name == "bot_yoked_replay":
        return "bot_yoked_replay_full", "bot_yoked_replay", "bot_yoked_replay"
...
def _make_bot_record(sample: Dict[str, Any]) -> Dict[str, Any]:
    rec = dict(sample.get("record") or {})
    # DeterministicConversationBot accepts these field names directly.
    return {
        "tick": rec.get("tick"),
        "true_top1_phrase": rec.get("top1_phrase"),
        "true_top1_family": rec.get("top1_family"),
        "true_top1_leaf": rec.get("top1_leaf"),
...
def _packet_to_emitted_item(packet: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "family": packet.get("input_family", "unknown"),
        "leaf": packet.get("input_leaf", ""),
        "id": packet.get("rule_id", "bot"),
        "form": "deterministic_bot_reply",
        "strength": "bot",
    }
...
def run_one_burst(args, cfg: Dict[str, Any], run_name: str) -> Dict[str, Any]:
    ...
    lag_bot = DeterministicConversationBot()
    ...
    yoked_packets = []
    if run_name == "bot_yoked_replay":
        yoked_packets = _load_yoked_packets(suite_dir / "bot_matched_trace")
        ...
        with base.ScanFirewall(C):
            for tick in range(start_tick, end_tick):
                ...
                for ev in sel["emitted"]:
                    ...
                    if policy == "no_return":
                    ...
                    elif policy == "bot_matched_trace":
                        choice = chooser.choose("fused", samples, tick)
                        bot_packet = bot.step(_make_bot_record(true_fused)).to_dict()
                        bot_source = "current_true_fused"
                        ...
                        kind = "deterministic_bot_reply"
                    elif policy == "bot_lagged_trace":
                        choice = chooser.choose("fused", samples, tick)
                        lag = int(cfg.get("bot_event_lag", 50))
                        if len(bot_lag_events) >= lag:
                            lag_sample = list(bot_lag_events)[-lag]
                            bot_packet = lag_bot.step(_make_bot_record(lag_sample)).to_dict()
                            bot_source = f"lagged_true_fused_{lag}_events"
                            bot_source_tick = (lag_sample.get("record") or {}).get("tick", "")
                        else:
                            bot_packet = _neutral_bot_packet(lag_bot, tick, f"lag_warmup_{len(bot_lag_events)}_of_{lag}")
                            bot_source = "neutral_lag_warmup"
                        ...
                        payload = str(bot_packet.get("reply_text", ""))
                        kind = "deterministic_bot_reply"
                    elif policy == "bot_yoked_replay":
                        choice = chooser.choose("fused", samples, tick)
                        if witness_index < len(yoked_packets):
                            src = yoked_packets[witness_index]
                            bot_packet = dict(src.get("bot_packet") or {})
                            bot_source = "yoked_replay_from_bot_matched_trace"
                            bot_source_tick = src.get("tick", "")
                            bot_source_witness_index = src.get("witness_index", witness_index)
                        else:
                            bot_packet = _neutral_bot_packet(bot, tick, "yoked_stream_exhausted")
                    ...
                    emitted=choice["emitted"] or {}
                    if payload:
                        prefix="self witness " if kind == "witness_code" else ("bot reply " if kind == "deterministic_bot_reply" else "self utterance ")
...
def main() -> int:
    ap=argparse.ArgumentParser(description="Trace-conditioned bot comparison suite core runner.")
