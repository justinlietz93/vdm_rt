...
Module: vdm_rt.core.cortex.maps.inhibitionmap
...
class InhibitionMap(BaseDecayMap):
    ...
    def __init__(
        ...
        spike_gain = config_float("maps.inhibition.spike_gain", 1.0) if spike_gain is None else float(spike_gain)
        dW_gain = config_float("maps.inhibition.delta_w_gain", 0.5) if dW_gain is None else float(dW_gain)
